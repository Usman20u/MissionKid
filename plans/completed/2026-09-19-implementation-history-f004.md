...
`useCurrentLocalPeriod` read the clock twice as the view arrived: the state initializer read it during the first render to seed the period, and the effect read it again on setup, but only to measure the wait from — `schedule(readClock.current())`. Nothing published that second reading. A local month boundary crossed between the two therefore split them: the period on screen came from the month that had just ended, while the wait was measured from the month that had just begun. The record then stated the ended month until something re-read the clock. What re-read it first, under ordinary clock progression and timer delivery, was the capped timeout: a wait toward the next boundary that exceeds `2 147 483 647` ms is taken in hops of that size, so the first hop re-reads the clock after about `24.855` days and the correct month is displayed from then on. A `focus` or `visibilitychange` return re-reads it sooner. The defect was therefore a wrong period label and a wrong figure for up to roughly twenty-five days after the boundary, not for a whole month, and against `F004`'s Monthly period clause and acceptance criterion 7 that is still the twenty-completion target counted against a month the family had already left.

The failure was observed on the production build before the fix, with the injected clock crossing `2024-03-31 23:59:59.999` to `2024-04-01 00:00:00.000` between those two readings. The record opened on `March 2024` at `2 / 20`, and the wait it scheduled was the clamped `2 147 483 647` ms of a wait toward `2024-05-01`, not the `1` ms that separated the first reading from the boundary. That is the defect itself, and it is what the browser runs observed.

The rest of that observed sequence is a property of the clock it was forced with, not of the implementation in ordinary use. One millisecond later, at the moment April began, the record still read `March 2024` at `2 / 20`; the clock was then moved straight to `2024-05-01` before the capped hop came due, so when the hop re-read it the month it found was May, and `May 2024` at `0 / 20` was the next month that run displayed. With a clock that advances by itself, the same hop would have re-read it at about `24.855` days — inside April — and displayed April. The recorded sequence is accurate as an observation of a forced clock and must not be read as the general behaviour.
...
One assertion holds it, through the rendered application and the real persistence adapter: the clock crosses the boundary between the view's two readings on entry, and the record must then name April with April's `0 / 20` while both March completions keep their places and their stored periods, with zero writes; it must then reach `May 2024` when the clock is carried to April's own boundary, so the refresh chain continues from the month the record is actually showing.

**Corrected after review.** This section first claimed that the wrong month stood until the month after the current one began, and that April "was never displayed at all". Both were wrong as general statements. They described the run's forced clock rather than the implementation: the capped hop re-reads the clock about `24.855` days after the boundary, and a return to the page re-reads it sooner, so the correct month appears then. The severity above is corrected accordingly, the observed sequence is kept with its proper scope, and the comment in `MissionHistory.tsx` that repeated the same claim is corrected with it. The defect and the fix are unchanged by this; only the duration claimed for the defect was wrong.
...
| 20 | Task 8 — the refresh taking two separate clock readings instead of one | green | behaviour-changing, **not caught** |

**20 mutations have now been run in total: 15 behaviour-changing mutations caught by the suite; 1 behaviour-changing mutation that survived it; 2 behaviourally equivalent mutations, correctly not caught; and 2 unchanged-behaviour controls, which left the suite green as controls are meant to.** The killed count is therefore 15, not the ten the earliest record claimed and not the 20 runs it took to establish them, and 16 of the 20 runs changed behaviour, of which one is not covered by any assertion. Of these, 11 cover the first batch — 9 caught, 1 equivalent, 1 control — and 9 cover Task 8, which is 6 caught, 1 survivor, 1 equivalent and 1 control. Rows 1 to 18 are the runs recorded before the entry-race correction; rows 19 and 20 are that correction's own.
...
**On the surviving mutation, row 20.** It was first recorded here as a third equivalent, on the reasoning that two adjacent statements cannot be put on opposite sides of a month boundary by any real clock. Independent review rejected that reasoning and it is withdrawn. Two adjacent readings can straddle midnight: the first can fall in the last millisecond of March and the second in the first millisecond of April, in which case the period shown is taken from March while the wait is measured from April — the same split this correction exists to remove, on a narrower window. The mutation is therefore behaviour-changing, and its green run is evidence that the suite does not cover it: the assertions here force exactly one crossing, between the render and the effect, so a second crossing inside the refresh is not exercised.

This is recorded as a known, uncovered mutation rather than closed. No further mutation campaign was run for it, and no assertion was added, so the suite's coverage of the refresh is what these runs show it to be and no more. What the shipped implementation does is take one reading and use it for both purposes, which is the property the mutation removes.
